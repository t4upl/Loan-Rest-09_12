...
public class ProductServiceTest extends AbstractTest {
  ...
  public void setUp() {
    ...
    this.customerService = Mockito
      .mock(CustomerService.class, new NullPointerExceptionAnswer("customerService"));
    ...
    this.decisionSystem = Mockito
      .mock(DecisionSystem.class, new NullPointerExceptionAnswer("decisionSystem"));
  ...
  public void whenClientDataValidReturnProduct() {
    ...
    when(abstractFactory.getProductFactory().getProduct(any(), any(), any(), any()))
          .thenAnswer((Answer<Product>) inv ->
            new Product(inv.getArgument(0), inv.getArgument(1),
  ...
  @Test
  public void whenExtendLoanAndLoanNotInDatabseReturnOptionalEmpty() {
    //given
  ...
  @Test
  public void whenExtendLoanAndLoanInDatabseReturnProduct() {
    //given
    ...
    doAnswer(invocationOnMock -> invocationOnMock.getArgument(0)).when(productRepository)
      .save(any());
    ...
    Assert.assertEquals("Product should have productSettings set by external srvice: "
        + " 'addExtensionTermToDueDate' method", productSettings.size(),
        optionalProduct.get().getProductSettings().size());
  }
